...
export const SubchapterModulesGrid: React.FC<SubchapterModulesGridProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Control Panel */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-3xl p-3.5 sm:p-5 shadow-m3-1">

        {/* 4 Filter Tabs */}
        <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-1 md:pb-0 no-scrollbar">
          <button
            onClick={() => setFilterMode('all')}
            className={`px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-2xl text-xs font-semibold transition-all flex items-center gap-1.5 whitespace-nowrap flex-shrink-0 ${
              filterMode === 'all'
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between gap-2 sm:gap-4">
                    ...
                    >
                      {isExpanded ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                      <span className="hidden sm:inline">{isExpanded ? 'Collapse' : 'Work Through'}</span>
                    </button>
                  </div>
                  ...
                  {parsedOptions.length > 0 && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {parsedOptions.map((opt, oIdx) => (
                        <div key={oIdx} className="p-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-300 text-xs flex items-center gap-2 overflow-x-auto">
                          ...
                          <div className="flex-1 min-w-0"><MathRenderer content={opt} /></div>
                        </div>
                      ))}
                    </div>
                  )}
                  ...
                  {isExpanded && (
                    <div className="space-y-4 pt-3 border-t border-slate-100 dark:border-slate-800 animate-in fade-in duration-200">
                      ...
                      {isSolRevealed && (
                        <div className="p-4 rounded-2xl bg-amber-50/60 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-800 space-y-2 animate-in fade-in duration-200 overflow-x-auto">
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3 sm:gap-4">
                    <div className="flex items-start gap-2.5 sm:gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <h3 className="text-sm sm:text-base font-bold text-slate-900 dark:text-white tracking-tight break-words">{exSet.title}</h3>
                        {exSet.description && (
                          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 line-clamp-2">{exSet.description}</p>
                        )}
                      </div>
                    </div>
                    <Link
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3">
                    ...
                    <div className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-800 space-y-0.5 text-center sm:col-span-2">
                      <div className="text-xs font-bold text-slate-700 dark:text-slate-300 font-mono truncate">{countLabel}</div>
